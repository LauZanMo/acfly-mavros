...
MAVConnSerial::MAVConnSerial(
    ...
    try {
        ...
        // Flow control setting in older versions of Boost.ASIO is broken, use workaround (below)
        // for now.
        serial_dev.set_option(
            SPB::flow_control((hwflow) ? SPB::flow_control::hardware : SPB::flow_control::none));
...
void MAVConnSerial::do_read(void) {
    ...
    serial_dev.async_read_some(buffer(rx_buf), [sthis](error_code error, size_t bytes_transferred) {
        if (error) {
...
void MAVConnSerial::do_write(bool check_tx_state) {
    ...
    serial_dev.async_write_some(buffer(buf_ref.dpos(), buf_ref.nbytes()),
