...
void MAVConnTCPClient::client_connected(size_t server_channel) {
    CONSOLE_BRIDGE_logInform(PFXd "Got client, id: %zu, address: %s", server_channel, conn_id,
...
void MAVConnTCPClient::do_recv() {
    ...
    socket.async_receive(buffer(rx_buf), [sthis](error_code error, size_t bytes_transferred) {
        if (error) {
...
void MAVConnTCPClient::do_send(bool check_tx_state) {
    ...
    socket.async_send(buffer(buf_ref.dpos(), buf_ref.nbytes()),
...
mavlink_status_t MAVConnTCPServer::get_status() {
    ...
    for (auto &instp : client_list) {
        ...
        // [[[cog:
        // for f in ('packet_rx_success_count', 'packet_rx_drop_count', 'buffer_overrun',
        // 'parse_error'):
...
void MAVConnTCPServer::do_accept() {
    ...
    acceptor.async_accept(acceptor_client->socket, acceptor_client->server_ep,
...
void MAVConnTCPServer::client_closed(std::weak_ptr<MAVConnTCPClient> weak_instp) {
    if (auto instp = weak_instp.lock()) {
        CONSOLE_BRIDGE_logInform(PFXd "Client connection closed, id: %p, address: %s", conn_id,
