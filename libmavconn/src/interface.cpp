...
void MAVConnInterface::log_recv(const char *pfx, mavlink_message_t &msg, Framing framing) {
    ...
    CONSOLE_BRIDGE_logDebug("%s%zu: recv: %s %4s Message-Id: %u [%u bytes] IDs: %u.%u Seq: %u", pfx,
...
void MAVConnInterface::log_send(const char *pfx, const mavlink_message_t *msg) {
    ...
    CONSOLE_BRIDGE_logDebug("%s%zu: send: %s Message-Id: %u [%u bytes] IDs: %u.%u Seq: %u", pfx,
...
void MAVConnInterface::send_message_ignore_drop(const mavlink::Message &msg,
    ...
    try {
        ...
        CONSOLE_BRIDGE_logError(PFX "%zu: DROPPED Message %s: %s", conn_id, msg.get_name().c_str(),
                                e.what());
    }
}
...
static MAVConnInterface::Ptr url_parse_udp(std::string hosts,
    ...
    url_parse_host(remote_pair, remote_host, remote_port, MAVConnUDP::DEFAULT_REMOTE_HOST,
                   MAVConnUDP::DEFAULT_REMOTE_PORT);
    url_parse_query(query, system_id, component_id);

    if (is_udpb)
        remote_host = permanent_broadcast ? MAVConnUDP::PERMANENT_BROADCAST_REMOTE_HOST
                                          : MAVConnUDP::BROADCAST_REMOTE_HOST;

    return std::make_shared<MAVConnUDP>(system_id, component_id, bind_host, bind_port, remote_host,
                                        remote_port);
}
...
MAVConnInterface::open_url_no_connect(std::string url, uint8_t system_id, uint8_t component_id) {
    ...
    CONSOLE_BRIDGE_logDebug(PFX "URL: %s: proto: %s, host: %s, path: %s, query: %s", url.c_str(),
...
MAVConnInterface::Ptr MAVConnInterface::open_url(std::string       url,
    ...
    if (interface_ptr) {
        if (!cb_handle_message) {
            CONSOLE_BRIDGE_logWarn(PFX "You did not provide message handling callback to "
                                       "open_url(), it is unsafe to set it later.");
        }
