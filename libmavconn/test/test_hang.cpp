 ...
 *     -L/opt/ros/kinetic/lib -L$HOME/ros/devel/lib -lroscpp -lrosconsole -lboost_system -lmavconn
 * -lrt \ -DMAVLINK_DIALECT=ardupilotmega
 */
...
static void send_sys_status(MAVConnInterface *ip) {
    ...
    st.serialize(map);
    mavlink::mavlink_finalize_message(&msg, ip->get_system_id(), ip->get_component_id(),
                                      st.MIN_LENGTH, st.LENGTH, st.CRC_EXTRA);
...
int main(int argc, char **argv) {
    ...
    client = MAVConnInterface::open_url("udp://:45001@localhost:45000", 44, 200,
                                        [&](const mavlink_message_t *msg, const Framing framing) {
                                            ...
                                            ROS_INFO("RECV: MsgID: %4u St: %d", msg->msgid,
                                                     int(framing));
                                        });
