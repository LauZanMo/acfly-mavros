...
public:
    OffboardControl()
        : nh_sp(test.nh),
          local_pos_sp_pub(
              nh_sp.advertise<geometry_msgs::PoseStamped>("/mavros/setpoint_position/local", 10)),
          vel_sp_pub(nh_sp.advertise<geometry_msgs::TwistStamped>(
              "/mavros/setpoint_velocity/cmd_vel", 10)),
          local_pos_sub(nh_sp.subscribe(
              "/mavros/local_position/local", 10, &OffboardControl::local_pos_cb, this)),
    ...
    void init() {
        ...
        if (use_pid) {
             ...
             * https://bitbucket.org/enddl22/ardrone_side_project/ tweaks to them so to get a better
             * velocity response are welcomed!
             */
            ...
            pid.setup_linvel_pid(linvel_p_gain, linvel_i_gain, linvel_d_gain, linvel_i_max,
                                 linvel_i_min, nh_sp);
            pid.setup_yawrate_pid(yawrate_p_gain, yawrate_i_gain, yawrate_d_gain, yawrate_i_max,
                                  yawrate_i_min, nh_sp);
        }
        ...
        if (mode_ == "position")
            ...
            ROS_ERROR_NAMED("sitl_test", "Control mode: wrong/unexistant control mode name %s",
                            mode_.c_str());
            return;
        }
        ...
        if (shape_ == "square")
            ...
            ROS_ERROR_NAMED("sitl_test", "Path shape: wrong/unexistant path shape name %s",
                            shape_.c_str());
            return;
        }
    }
...
private:
    ...
    Eigen::Vector3d pos_setpoint(int tr_x, int tr_y, int tr_z) {
        /** @todo Give possibility to user define amplitude of movement (square corners
         * coordinates)*/
    ...
    Eigen::Vector3d eight_shape(int angle) {
        ...
        return Eigen::Vector3d(
            a * cos(angles::from_degrees(angle)),
            a * sin(angles::from_degrees(angle)) * cos(angles::from_degrees(angle)), 1.0f);
    }
        ...
        while (ros::ok()) {
            ...
            if (mode == POSITION) {
                ...
                if (use_pid)
                    tf::vectorEigenToMsg(pid.compute_linvel_effort(
                                             Eigen::Vector3d(5.0f, 0.0f, 1.0f), current, last_time),
                                         vs.twist.linear);
                else
                    tf::vectorEigenToMsg(
                        Eigen::Vector3d(5.0f - current.x(), -current.y(), 1.0f - current.z()),
                        vs.twist.linear);
            ...
            for (int theta = 0; theta <= 360; theta++) {
                ...
                if (mode == POSITION) {
                    ...
                    if (use_pid)
                        tf::vectorEigenToMsg(
                            pid.compute_linvel_effort(circle_shape(theta), current, last_time),
                            vs.twist.linear);
                    else
    ...
    void eight_path_motion(ros::Rate loop_rate, control_mode mode) {
        ...
        while (ros::ok()) {
            ...
            if (mode == POSITION) {
                ...
                if (use_pid)
                    tf::vectorEigenToMsg(pid.compute_linvel_effort(
                                             Eigen::Vector3d(0.0f, 0.0f, 1.0f), current, last_time),
                                         vs.twist.linear);
                else
                    tf::vectorEigenToMsg(
            ...
            for (int theta = -180; theta <= 180; theta++) {
                ...
                if (mode == POSITION) {
                    ...
                    if (use_pid)
                        tf::vectorEigenToMsg(
                            pid.compute_linvel_effort(eight_shape(theta), current, last_time),
                            vs.twist.linear);
                    else
    ...
    void ellipse_path_motion(ros::Rate loop_rate, control_mode mode) {
        ...
        while (ros::ok()) {
            ...
            if (mode == POSITION) {
                ...
                if (use_pid)
                    tf::vectorEigenToMsg(pid.compute_linvel_effort(
                                             Eigen::Vector3d(0.0f, 0.0f, 2.5f), current, last_time),
                                         vs.twist.linear);
                else
                    tf::vectorEigenToMsg(
            ...
            for (int theta = 0; theta <= 360; theta++) {
                ...
                if (mode == POSITION) {
                    ...
                    if (use_pid)
                        tf::vectorEigenToMsg(
                            pid.compute_linvel_effort(ellipse_shape(theta), current, last_time),
                            vs.twist.linear);
                    else
    ...
    void wait_and_move(geometry_msgs::PoseStamped target) {
        ...
        while (ros::ok() && !stop) {
            ...
            if (mode == POSITION) {
                ...
                if (use_pid)
                    tf::vectorEigenToMsg(pid.compute_linvel_effort(dest, current, last_time),
                                         vs.twist.linear);
                else
