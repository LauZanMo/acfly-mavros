...
#ifdef CONTROL_TOOLBOX_PRE_1_14
    pid_yaw_rate.initPid(yawrate_pid[0], yawrate_pid[1], yawrate_pid[2], yawrate_imax, yawrate_imin,
                         node);
#else
    pid_yaw_rate.initPid(yawrate_pid[0], yawrate_pid[1], yawrate_pid[2], yawrate_imax, yawrate_imin,
                         false, node);
...
Eigen::Vector3d PIDController::compute_linvel_effort(Eigen::Vector3d goal,
                                                     ...
                                                     ros::Time       last_time) {
    double lin_vel_x =
        pid_linvel_x.computeCommand(goal.x() - current.x(), ros::Time::now() - last_time);
    double lin_vel_y =
        pid_linvel_y.computeCommand(goal.y() - current.y(), ros::Time::now() - last_time);
    double lin_vel_z =
        pid_linvel_z.computeCommand(goal.z() - current.z(), ros::Time::now() - last_time);
