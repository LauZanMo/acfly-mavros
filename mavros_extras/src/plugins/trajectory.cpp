...
public:
    TrajectoryPlugin() : PluginBase(), trajectory_nh("~trajectory") {}

    void initialize(UAS &uas_) override {
        PluginBase::initialize(uas_);

        trajectory_generated_sub =
            trajectory_nh.subscribe("generated", 10, &TrajectoryPlugin::trajectory_cb, this);
...
private:
    ...
    //     cog.outl(
    //         """void fill_points_{vec_name}(MavPoints &{x}, MavPoints &{y}, MavPoints &{z}, const
    //         geometry_msgs::{vec_type} &{vec_name}, const size_t i)\n"""
    //         """{{\n"""
    //         """\tauto {vec_name}_ned =
    //         ftf::transform_frame_enu_ned(ftf::to_eigen({vec_name}));\n""" .format(**locals())
    //     )
    //
    //     for axis in "xyz":
    ...
    auto fill_points_unused_path(mavlink::common::msg::TRAJECTORY_REPRESENTATION_WAYPOINTS &t,
                                 const size_t                                               i) {
        t.vel_x[i]   = NAN;
        t.vel_y[i]   = NAN;
        t.vel_z[i]   = NAN;
    ...
    void fill_points_all_unused(mavlink::common::msg::TRAJECTORY_REPRESENTATION_WAYPOINTS &t,
                                const size_t                                               i) {
        t.pos_x[i] = NAN;
        t.pos_y[i] = NAN;
        t.pos_z[i] = NAN;
    ...
    void fill_points_all_unused_bezier(mavlink::common::msg::TRAJECTORY_REPRESENTATION_BEZIER &t,
                                       const size_t                                            i) {
        t.pos_x[i] = NAN;
        t.pos_y[i] = NAN;
        t.pos_z[i] = NAN;
    ...
    void fill_msg_position(geometry_msgs::Point &position,
    ...
    void fill_msg_velocity(geometry_msgs::Vector3 &velocity,
                           const mavlink::common::msg::TRAJECTORY_REPRESENTATION_WAYPOINTS &t,
                           const size_t                                                     i) {
        auto velocity_enu =
    ...
    void fill_msg_acceleration(geometry_msgs::Vector3 &acceleration,
                               const mavlink::common::msg::TRAJECTORY_REPRESENTATION_WAYPOINTS &t,
                               const size_t                                                     i) {
        ...
        if (req->type == mavros_msgs::Trajectory::MAV_TRAJECTORY_REPRESENTATION_WAYPOINTS) {
            ...
            auto fill_point_rep_waypoints =
                [&](mavlink::common::msg::TRAJECTORY_REPRESENTATION_WAYPOINTS &t,
                    ...
                    fill_points_acceleration(t.acc_x, t.acc_y, t.acc_z, rp.acceleration_or_force,
                                             i);
    ...
    void path_cb(const nav_msgs::Path::ConstPtr &req) {
        ...
        auto fill_point = [&](mavlink::common::msg::TRAJECTORY_REPRESENTATION_WAYPOINTS &t,
                              const size_t                                               i) {
            t.command[i] = UINT16_MAX;
