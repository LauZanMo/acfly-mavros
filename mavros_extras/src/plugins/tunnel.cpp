    ...
    void ros_callback(const mavros_msgs::Tunnel::ConstPtr &ros_tunnel) {
        try {
            const auto mav_tunnel =
                copy_tunnel<mavros_msgs::Tunnel, mavlink::common::msg::TUNNEL>(*ros_tunnel);
    ...
    void mav_callback(const mavlink::mavlink_message_t *,
        ...
        try {
            const auto ros_tunnel =
                copy_tunnel<mavlink::common::msg::TUNNEL, mavros_msgs::Tunnel>(mav_tunnel);
...
PLUGINLIB_EXPORT_CLASS(mavros::extra_plugins::TunnelPlugin, mavros::plugin::PluginBase)
