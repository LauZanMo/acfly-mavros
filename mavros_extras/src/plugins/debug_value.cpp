...
public:
    DebugValuePlugin() : PluginBase(), debug_nh("~debug_value") {}
    ...
    void initialize(UAS &uas_) override {
        ...
        named_value_float_pub =
            debug_nh.advertise<mavros_msgs::DebugValue>("named_value_float", 10);
...
private:
    ...
    void debug_logger(const std::string &type, const mavros_msgs::DebugValue &dv) {
        ...
        ROS_DEBUG_STREAM_NAMED("debug_value", type << "\t" << dv.header.stamp << "\t" << name
                                                   << "\t[" << dv.index << "]\tvalue:" << ss.str());
    }
    ...
    void handle_debug(const mavlink::mavlink_message_t *msg, mavlink::common::msg::DEBUG &debug) {
        ...
        //
        //     cog.outl("""auto {p} =
        //     boost::make_shared<mavros_msgs::DebugValue>();""".format(**_args))
        //     cog.outl("""{p}->header.stamp =
        //     m_uas->synchronise_stamp({val}.{time_f});""".format(**_args)) cog.outl("""{p}->type =
        //     mavros_msgs::DebugValue::{type_};""".format(**_args)) cog.outl("""{p}->index =
        //     {index};""".format(**_args)) if name:
