...
public:
    MocapPoseEstimatePlugin() : PluginBase(), mp_nh("~mocap") {}
    ...
    void initialize(UAS &uas_) override {
        ...
        if (use_tf && !use_pose) {
            ...
            mocap_pose_sub =
                mp_nh.subscribe("pose", 1, &MocapPoseEstimatePlugin::mocap_pose_cb, this);
        } else {
...
private:
    ...
    void mocap_pose_cb(const geometry_msgs::PoseStamped::ConstPtr &pose) {
        ...
        auto q =
            ftf::transform_orientation_enu_ned(ftf::transform_orientation_baselink_aircraft(q_enu));

        auto position = ftf::transform_frame_enu_ned(
            Eigen::Vector3d(pose->pose.position.x, pose->pose.position.y, pose->pose.position.z));

        mocap_pose_send(pose->header.stamp.toNSec() / 1000, q, position);
    }
    ...
    void mocap_tf_cb(const geometry_msgs::TransformStamped::ConstPtr &trans) {
        ...
        auto position = ftf::transform_frame_enu_ned(
            Eigen::Vector3d(trans->transform.translation.x, trans->transform.translation.y,
                            trans->transform.translation.z));

        mocap_pose_send(trans->header.stamp.toNSec() / 1000, q, position);
    }
};
} // namespace extra_plugins
} // namespace mavros
