 ...
 *
 * This plugin is following ROS REP 147. Pose is expressed in parent frame. (Quaternion rotates from
 * child to parent) The twist is expressed in the child frame.
 *
...
private:
    ...
    std::string
        fcu_odom_parent_id_des; //!< desired orientation of the fcu odometry message's parent frame
    std::string
        fcu_odom_child_id_des; //!< desired orientation of the fcu odometry message's child frame
    ...
    void lookup_static_transform(const std::string &target,
        ...
        try {
            ...
            tf_source2target = tf2::transformToEigen(
                m_uas->tf2_buffer.lookupTransform(target, source, ros::Time(0)));
     ...
     *
     * Callback for mavlink ODOMETRY messages sent from the FCU. According to the mavlink
     * specification, all quantities are for the child frame (fcu_frd), expressed in the parent
     * frame (local_origin_ned). To be compliant with ROS REP 147 for the published
     * nav_msgs/Odometry, the data will be appropriately transformed and published. Frames for the
     * publish message should be specified in specified in the rosparams
     * "odometry/fcu/odom_*_id_des" (set in px4_config.yaml).
     *
    ...
    void handle_odom(const mavlink::mavlink_message_t *msg,
        ...
        Matrix6d           r_pose =
            Matrix6d::Zero(); //!< Zero initialized pose 6-D Covariance matrix. WRT frame_id
        Matrix6d r_vel = Matrix6d::Zero(); //!< Zero initialized velocity 6-D Covariance matrix. WRT
                                           //!< child_frame_id
        ...
        /**
         * Orientation parsing. Quaternion has to be the rotation from desired child frame to
         * desired parent frame
         */
        ...
        ang_vel = Eigen::Vector3d(tf_child2child_des.linear() * Eigen::Vector3d(odom_msg.rollspeed,
                                                                                odom_msg.pitchspeed,
                                                                                odom_msg.yawspeed));
        ...
        Eigen::Map<Matrix6d>(odom->pose.covariance.data(), cov_pose.rows(), cov_pose.cols()) =
            cov_pose;
        ...
        Eigen::Map<Matrix6d>(odom->twist.covariance.data(), cov_vel.rows(), cov_vel.cols()) =
            cov_vel;
    ...
    void odom_cb(const nav_msgs::Odometry::ConstPtr &odom) {
        ...
        ROS_DEBUG_STREAM_NAMED("odom", "ODOM: output: pose covariance matrix:" << std::endl
                                                                               << cov_pose_map);
        ROS_DEBUG_STREAM_NAMED("odom", "ODOM: output: velocity covariance matrix:" << std::endl
                                                                                   << cov_vel_map);
