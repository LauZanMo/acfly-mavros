...
public:
    ObstacleDistancePlugin() : PluginBase(), obstacle_nh("~obstacle") {}
...
private:
    ...
    void obstacle_cb(const sensor_msgs::LaserScan::ConstPtr &req) {
        ...
        if (req->ranges.size() <= obstacle.distances.size()) {
            ...
            std::fill(obstacle.distances.begin() + req->ranges.size(), obstacle.distances.end(),
                      UINT16_MAX); //!< fill the rest of the array values as "Unknown"

            const float increment_deg = req->angle_increment * RAD_TO_DEG;
            obstacle.increment =
                static_cast<uint8_t>(increment_deg + 0.5f); //!< Round to nearest integer.
            obstacle.increment_f = increment_deg;
        } else {
            // all distances from sensor will not fit so we combine adjacent distances always taking
            ...
            obstacle.increment =
                ceil(req->angle_increment * RAD_TO_DEG * scale_factor); //!< [degrees]
        }

        obstacle.time_usec   = req->header.stamp.toNSec() / 1000; //!< [microsecs]
        obstacle.sensor_type = utils::enum_value(
            MAV_DISTANCE_SENSOR::LASER); //!< defaults is laser type (depth sensor, Lidar)
        ...
        ROS_DEBUG_STREAM_NAMED(
            "obstacle_distance",
            "OBSDIST: sensor type: "
                << utils::to_string_enum<MAV_DISTANCE_SENSOR>(obstacle.sensor_type) << std::endl
                << obstacle.to_yaml());
