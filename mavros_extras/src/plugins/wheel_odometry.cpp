...
public:
    ...
    void initialize(UAS &uas_) override {
        ...
        // Read parameters for each wheel.
        {
            ...
            if (wheel_offset.size() >= count) {
                ...
                if (wheel_radius.size() == 1) {
                    ...
                    wheel_offset[1].y() =
                        wheel_offset[0].y() + 1.0; // make separation non-zero to avoid div-by-zero
                ...
                if (separation < 1.e-5) {
                    odom_mode = OM::NONE;
                    ROS_WARN_NAMED("wo",
                                   "WO: Separation between the first two wheels is too small (%f).",
                                   separation);
                }
                ...
                for (int i = 0; i < wheel_radius.size(); i++) {
                    if (wheel_radius[i] <= 1.e-5) {
                        odom_mode = OM::NONE;
                        ROS_WARN_NAMED("wo", "WO: Wheel #%i has incorrect radius (%f).", i,
                                       wheel_radius[i]);
                    }
                }
            } else {
                odom_mode = OM::NONE;
                ROS_WARN_NAMED("wo", "WO: Not all wheels have parameters specified (%lu/%i).",
        ...
        if (odom_mode != OM::NONE) {
            if (twist_send)
                twist_pub =
                    wo_nh.advertise<geometry_msgs::TwistWithCovarianceStamped>("velocity", 10);
            else
...
private:
     ...
     * Odometry is computed from the very start but no pose info is published until we have initial
     * orientation (yaw). Once we get it, the robot's current pose is updated with it and starts to
     * be published. Twist info doesn't depend on initial orientation so is published from the very
     * start.
    ...
    void publish_odometry(ros::Time time) {
        ...
        if (!yaw_initialized && m_uas->get_attitude_imu_enu()) {
            double yaw =
                ftf::quaternion_get_yaw(ftf::to_eigen(m_uas->get_attitude_orientation_enu()));
     ...
     * The wheels are assumed to be parallel to the robot's x-direction (forward) and with the same
     * x-offset. No slip is assumed (Instantaneous Center of Curvature (ICC) along the axis
     * connecting the wheels). All computations are performed for ROS frame conventions. The
     * approach is the extended and more accurate version of standard one described in the book
     ...
     * and that the robot's origin can be specified anywhere instead of the middle-point between the
     * wheels.
    ...
    void update_odometry_diffdrive(std::vector<double> distance, double dt) {
        ...
        if (rtwist_cov(0) == 0.0) {
            ...
            rtwist_cov(1) = vel_cov * a * a * 2.0 * dy_inv * dy_inv +
                            0.001; // vy_cov (add extra error, otherwise vy_cov= 0 if a=0)
        ...
        // Update pose cov
        rpose_cov =
            J_pose * rpose_cov * J_pose.transpose() + J_meas * meas_cov * J_meas.transpose();
    }
        ...
        // Compute odometry
            ...
            if (rpm) {
                for (int i = 0; i < count; i++) {
                    ...
                    double rpm         = 0.5 * (measurement[i] +
                                        measurement_prev[i]); // Mean RPM during last dt seconds
            ...
            // Make distance of the 2nd wheel equal to that of the 1st one if requested or only one
            // is available. This generalizes odometry computations for 1- and 2-wheels
            // configurations.
            if (count == 1)
    ...
    void handle_wheel_distance(const mavlink::mavlink_message_t     *msg,
        ...
        ros::Time timestamp_int = ros::Time(wheel_dist.time_usec / 1000000UL,
