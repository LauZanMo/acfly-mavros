...
public:
    OnboardComputerStatusPlugin() : PluginBase(), status_nh("~onboard_computer") {}
...
private:
    ...
    void status_cb(const mavros_msgs::OnboardComputerStatus::ConstPtr &req) {
        ...
        //     'link_tx_max',
        //     'link_rx_max'):
        //     cog.outl("std::copy(req->%s.cbegin(), req->%s.cend(), status.%s.begin());" % (f, f,
        //     f))
        // ]]]
...
PLUGINLIB_EXPORT_CLASS(mavros::extra_plugins::OnboardComputerStatusPlugin,
