...
public:
    MountControlPlugin() : PluginBase(), nh("~"), mount_nh("~mount_control") {}
    ...
    void initialize(UAS &uas_) override {
        ...
        configure_srv =
            mount_nh.advertiseService("configure", &MountControlPlugin::mount_configure_cb, this);
    }
...
private:
     ...
     */
    void handle_mount_status(const mavlink::mavlink_message_t *,
        ...
        auto q = ftf::quaternion_from_rpy(
            Eigen::Vector3d(ms.pointing_b, ms.pointing_a, ms.pointing_c) * M_PI / 18000.0);
        geometry_msgs::Quaternion quaternion_msg;
        tf::quaternionEigenToMsg(q, quaternion_msg);
        mount_orientation_pub.publish(quaternion_msg);
    }
    ...
    bool mount_configure_cb(mavros_msgs::MountConfigure::Request  &req,
        ...
        ROS_ERROR_COND_NAMED(!res.success, "mount",
