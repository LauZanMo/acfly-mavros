...
public:
    ...
    void initialize(UAS &uas_) override {
        ...
        fp_nh.param("geo_origin/alt", origin_alt,
                    408.0); // [meters - height over the WGS-84 ellipsoid]
        ...
        fp_nh.param(
            "mocap_transform", mocap_transform,
            true); // listen to MoCap source (TransformStamped if true; PoseStamped if false)
        fp_nh.param("mocap_withcovariance", mocap_withcovariance,
                    false); // ~mocap/pose uses PoseWithCovarianceStamped Message
        ...
        if (use_mocap) {
            if (mocap_transform) { // MoCap data in TransformStamped msg
                ...
                mocap_pose_cov_sub =
                    fp_nh.subscribe("mocap/pose_cov", 10, &FakeGPSPlugin::mocap_pose_cov_cb, this);
            } else { // MoCap data in PoseStamped msg
                mocap_pose_sub =
                    fp_nh.subscribe("mocap/pose", 10, &FakeGPSPlugin::mocap_pose_cb, this);
            }
...
private:
    ...
    void send_fake_gps(const ros::Time &stamp, const Eigen::Vector3d &ecef_offset) {
        ...
        try {
            earth.Reverse(current_ecef.x(), current_ecef.y(), current_ecef.z(), geodetic.x(),
                          geodetic.y(), geodetic.z());
        } catch (const std::exception &e) {
            ROS_INFO_STREAM("FGPS: Caught exception: " << e.what() << std::endl);
        }
        ...
        if (use_hil_gps) {
            /**
             * @note: <a href="https://mavlink.io/en/messages/common.html#HIL_GPS">HIL_GPS MAVLink
             * message</a> is supported by both Ardupilot and PX4 Firmware. But on PX4 Firmware are
             * only acceped out of HIL mode if use_hil_gps flag is set (param MAV_USEHILGPS = 1).
             */
            ...
            hil_gps.alt       = (geodetic.z() + GeographicLib::Geoid::ELLIPSOIDTOGEOID *
                                              (*m_uas->egm96_5)(geodetic.x(), geodetic.y())) *
                          1e3;                               // [meters * 1e3]
            ...
            /**
             * @note: <a href="https://mavlink.io/en/messages/common.html#GPS_INPUT">GPS_INPUT
            ...
            gps_input.horiz_accuracy =
                horiz_accuracy; // [m] will either use the static parameter value, or the dynamic
                                // covariance from function mocap_pose_cov_cb() bellow
            gps_input.vert_accuracy =
                vert_accuracy; // [m] will either use the static parameter value, or the dynamic
                               // covariance from function mocap_pose_cov_cb() bellow
            gps_input.lat = geodetic.x() * 1e7; // [degrees * 1e7]
            gps_input.lon = geodetic.y() * 1e7; // [degrees * 1e7]
            gps_input.alt =
                (geodetic.z() + GeographicLib::Geoid::ELLIPSOIDTOGEOID *
    ...
    void mocap_tf_cb(const geometry_msgs::TransformStamped::ConstPtr &trans) {
        ...
        send_fake_gps(trans->header.stamp, ftf::transform_frame_enu_ecef(
    ...
    void mocap_pose_cov_cb(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &req) {
        ...
        send_fake_gps(req->header.stamp, ftf::transform_frame_enu_ecef(
                                             Eigen::Vector3d(pos_enu.translation()), map_origin));
    }
