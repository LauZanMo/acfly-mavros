...
public:
    VisionSpeedEstimatePlugin()
        : PluginBase(), sp_nh("~vision_speed"), listen_twist(true), twist_cov(true) {}
        ...
        if (listen_twist) {
            if (twist_cov)
                vision_twist_cov_sub = sp_nh.subscribe(
                    "speed_twist_cov", 10, &VisionSpeedEstimatePlugin::twist_cov_cb, this);
            else
                vision_twist_sub =
                    sp_nh.subscribe("speed_twist", 10, &VisionSpeedEstimatePlugin::twist_cb, this);
        } else
            vision_vector_sub =
