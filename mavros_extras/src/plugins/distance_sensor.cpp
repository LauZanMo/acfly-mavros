...
public:
    ...
    Eigen::Quaternionf
        quaternion; //!< sensor orientation in vehicle body frame for MAV_SENSOR_ROTATION_CUSTOM
...
public:
    DistanceSensorPlugin() : PluginBase(), dist_nh("~distance_sensor") {}
    ...
    void initialize(UAS &uas_) override {
        ...
        for (auto &pair : map_dict) {
            ROS_DEBUG_NAMED("distance_sensor", "DS: initializing mapping for %s",
                            pair.first.c_str());
...
void DistanceSensorItem::range_cb(const sensor_msgs::Range::ConstPtr &msg) {
    ...
    ROS_DEBUG_NAMED("distance_sensor", "DS: %d: sensor variance: %f", sensor_id,
                    calculate_variance(msg->range) * 1E2);
    ...
    owner->distance_sensor(msg->header.stamp.toNSec() / 1000000, msg->min_range / 1E-2,
                           msg->max_range / 1E-2, msg->range / 1E-2, type, sensor_id, orientation,
                           covariance_, msg->field_of_view * horizontal_fov_ratio,
                           msg->field_of_view * vertical_fov_ratio, q);
}
...
DistanceSensorItem::Ptr DistanceSensorItem::create_item(DistanceSensorPlugin *owner,
    ...
    if (!pnh.getParam("id", id)) {
        ...
        p.reset();
        return p; // return nullptr because of a bug related to gcc 4.6
    }
    p->sensor_id = id;
    ...
    if (!p->is_subscriber) {
        ...
        if (p->send_tf) { // sensor position defined if 'send_tf' set to TRUE
            ...
            ROS_DEBUG_NAMED("sensor_position", "DS: %s: Sensor position at: %f, %f, %f",
                            topic_name.c_str(), p->position.x(), p->position.y(), p->position.z());
        }
    } else {
        // subscriber params
