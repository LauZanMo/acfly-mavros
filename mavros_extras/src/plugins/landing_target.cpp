 ...
 * @brief Landing Target plugin
 *
 * This plugin is intended to publish the location of a landing area captured from a downward facing
 * camera to the FCU and/or receive landing target tracking data coming from the FCU.
 */
...
public:
    ...
    void initialize(UAS &uas_) override {
        ...
        frame = utils::mav_frame_from_str(mav_frame); // MAV_FRAME index based on given frame name
                                                      // (If unknown, defaults to GENERIC)

        nh.param<std::string>("land_target_type", land_target_type, "VISION_FIDUCIAL");
        type = utils::landing_target_type_from_str(
            land_target_type); // LANDING_TARGET_TYPE index based on given type
                               // name (If unknown, defaults to LIGHT_BEACON)
        ...
        if (listen_tf) { // Listen to transform
            ROS_INFO_STREAM_NAMED("landing_target", "Listen to landing_target transform "
                                                        << tf_frame_id << " -> "
                                                        << tf_child_frame_id);
...
private:
    ...
    void send_landing_target(const ros::Time &stamp, const Eigen::Affine3d &tr) {
        ...
        if (land_target_type.find("VISION")) {
             ...
             * The target size is computed by the angle of view formula (similar to angular
             * diameter).
             */
             ...
             * δ = 2 * atan(d / (2 * D))
             * where,	d = actual diameter; D = distance to the object (or focal length of a
             * camera)
             */
            size_rad = {2 * (M_PI / 180.0) * atan(target_size_x / (2 * focal_length)),
                        2 * (M_PI / 180.0) * atan(target_size_y / (2 * focal_length))};
        }
        // else, the same values are computed considering the displacement relative to X and Y axes
        // of the camera frame reference
        else {
        ...
        if (last_transform_stamp == stamp) {
            ROS_DEBUG_THROTTLE_NAMED(10, "landing_target",
        ...
        ROS_DEBUG_THROTTLE_NAMED(10, "landing_target",
                                 ...
                                 angle.x(), angle.y(), distance, pos.x(), pos.y(), pos.z(), rpy.x(),
                                 rpy.y(), rpy.z(), size_rad.x(), size_rad.y(),
                                 utils::to_string(static_cast<LANDING_TARGET_TYPE>(type)).c_str());

        landing_target(stamp.toNSec() / 1000, id,
    ...
    void handle_landing_target(const mavlink::mavlink_message_t     *msg,
        ...
        auto orientation = ftf::transform_orientation_aircraft_baselink(
            ftf::transform_orientation_ned_enu(ftf::mavlink_to_quaternion(land_target.q)));
        ...
        ROS_DEBUG_THROTTLE_NAMED(
            ...
            land_target.angle_x, land_target.angle_y, land_target.distance, position.x(),
            position.y(), position.z(), rpy.x(), rpy.y(), rpy.z(), land_target.size_x,
            land_target.size_y,
    ...
    void landtarget_cb(const mavros_msgs::LandingTarget::ConstPtr &req) {
        ...
        landing_target(req->header.stamp.toNSec() / 1000, req->target_num,
                       req->frame, // by default, in LOCAL_NED
                       Eigen::Vector2f(req->angle[0], req->angle[1]), req->distance,
                       Eigen::Vector2f(req->size[0], req->size[1]), position, orientation,
                       req->type,
                       1); // position is valid from the first received msg
    }
};
