...
static double waypoint_encode_factor(const uint8_t &frame) {
    ...
    // local_names = [v for v in all_names if v.startswith(('LOCAL', 'BODY', 'MOCAP', 'VISION',
    // 'ESTIM'))] other_names = ['MISSION']
    //
...
std::string waypoint_to_string(const ITEM &wp) {
    ...
    ss << '#' << wp.seq << (wp.current ? '*' : ' ') << " F:" << +wp.frame << " C:" << std::setw(3)
       << wp.command;
    ss << " p: " << wp.param1 << ' ' << wp.param2 << ' ' << wp.param3 << ' ' << wp.param4
...
public:
    ...
    virtual void initialize_with_nodehandle(ros::NodeHandle *_wp_nh) {
        ...
        schedule_timer =
            _wp_nh->createTimer(BOOTUP_TIME_DT, &MissionBase::scheduled_pull_cb, this, true);
        schedule_timer.stop();
    }
...
protected:
    ...
    void send_waypoint(size_t seq) {
        if (seq < send_waypoints.size()) {
            ...
            mission_send(wpi);
            ROS_DEBUG_STREAM_NAMED(log_ns,
                                   log_ns << ": send item " << waypoint_to_string<ITEM>(wpi));
        }
    }
    ...
    void mission_write_partial_list(uint16_t start_index, uint16_t end_index) {
        ROS_DEBUG_NAMED(log_ns, "%s:m: write partial list %u - %u", log_ns.c_str(), start_index,
                        end_index);
