...
public:
    ...
    void set_position_target_global_int(uint32_t time_boot_ms, uint8_t coordinate_frame,
        ...
        // [[[cog:
        // for f in ('time_boot_ms', 'coordinate_frame', 'type_mask', 'lat_int', 'lon_int', 'alt',
        // 'yaw', 'yaw_rate'):
        //     cog.outl("sp.%s = %s;" % (f, f))
    ...
    void tf2_start(const char *_thd_name, void (D::*cbp)(const geometry_msgs::TransformStamped &)) {
        ...
        tf_thread = std::thread([this, tf_transform_cb]() {
            ...
            while (ros::ok()) {
                ...
                if (m_uas_->tf2_buffer.canTransform(_frame_id, _child_frame_id, ros::Time(0),
                                                    ros::Duration(3.0))) {
                    try {
                ...
                if (m_uas_->tf2_buffer.canTransform(_frame_id, _child_frame_id, ros::Time(0),
                    ...
                    try {
                        ...
                        tf2_filter.registerCallback(
                            boost::bind(cbp, static_cast<D *>(this), transform, _1));
                    } catch (tf2::LookupException &ex) {
                        ROS_ERROR_NAMED("tf2_buffer", "%s: %s", tf_thd_name.c_str(), ex.what());
                    }
                }
