...
public:
    ...
    SetpointPositionPlugin()
        : PluginBase(), sp_nh("~setpoint_position"), spg_nh("~"), tf_listen(false), tf_rate(50.0) {}
    ...
    void initialize(UAS &uas_) override {
        ...
        if (tf_listen) {
            ...
            setpointg_sub =
                sp_nh.subscribe("global", 10, &SetpointPositionPlugin::setpointg_cb, this);
            // Subscriber for goal gps but will convert it to local coordinates
            setpointg2l_sub = sp_nh.subscribe("global_to_local", 10,
                                              &SetpointPositionPlugin::setpointg2l_cb, this);
            // subscriber for current gps state, mavros/global_position/global.
            gps_sub = spg_nh.subscribe("global_position/global", 10,
                                       &SetpointPositionPlugin::gps_cb, this);
            // Subscribe for current local ENU pose.
            local_sub = spg_nh.subscribe("local_position/pose", 10,
                                         &SetpointPositionPlugin::local_cb, this);
        }
        mav_frame_srv =
            sp_nh.advertiseService("mav_frame", &SetpointPositionPlugin::set_mav_frame_cb, this);
...
private:
    ...
    void setpointg_cb(const geographic_msgs::GeoPoseStamped::ConstPtr &req) {
        ...
        set_position_target_global_int(
            req->header.stamp.toNSec() / 1000000, uint8_t(MAV_FRAME::GLOBAL_INT), type_mask,
            req->pose.position.latitude * 1e7, req->pose.position.longitude * 1e7,
    ...
    void setpointg2l_cb(const geographic_msgs::GeoPoseStamped::ConstPtr &req) {
        ...
        earth.Forward(current_gps.x(), current_gps.y(), current_gps.z(), current_ecef.x(),
                      current_ecef.y(), current_ecef.z());

        // goal gps -> goal ECEF
        Eigen::Vector3d goal_ecef;
        earth.Forward(goal_gps.x(), goal_gps.y(), goal_gps.z(), goal_ecef.x(), goal_ecef.y(),
                      goal_ecef.z());
