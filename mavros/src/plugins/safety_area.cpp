...
public:
    SafetyAreaPlugin() : PluginBase(), safety_nh("~safety_area") {}
    ...
    void initialize(UAS &uas_) override {
        ...
        if (safety_nh.getParam("p1/x", p1x) && safety_nh.getParam("p1/y", p1y) &&
            safety_nh.getParam("p1/z", p1z)) {
            manual_def = true;
            ROS_DEBUG_NAMED("safetyarea", "SA: Manual set: P1(%f %f %f)", p1x, p1y, p1z);
        }

        if (manual_def && safety_nh.getParam("p2/x", p2x) && safety_nh.getParam("p2/y", p2y) &&
            safety_nh.getParam("p2/z", p2z)) {
            manual_def = true;
            ROS_DEBUG_NAMED("safetyarea", "SA: Manual set: P2(%f %f %f)", p2x, p2y, p2z);
        } else
            manual_def = false;

        if (manual_def)
            send_safety_set_allowed_area(Eigen::Vector3d(p1x, p1y, p1z),
    ...
    Subscriptions get_subscriptions() override {
        return {make_handler(&SafetyAreaPlugin::handle_safety_allowed_area)};
    }
...
private:
    ...
    void handle_safety_allowed_area(const mavlink::mavlink_message_t          *msg,
        ...
        // saa_msg->header.frame_id = utils::to_string(saa.frame);	@TODO: after #311 merged this
        // will work
