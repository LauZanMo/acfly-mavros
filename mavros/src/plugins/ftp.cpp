...
public:
    ...
    bool decode_valid(UAS *uas) {
        ...
        ROS_DEBUG_NAMED("ftp", "FTP:rm: SEQ(%u) SESS(%u) OPCODE(%u) RQOP(%u) SZ(%u) OFF(%u)",
                        hdr->seqNumber, hdr->session, hdr->opcode, hdr->req_opcode, hdr->size,
                        hdr->offset);
#endif
...
#ifdef FTP_LL_DEBUG
        ROS_DEBUG_NAMED("ftp", "FTP:sm: SEQ(%u) SESS(%u) OPCODE(%u) SZ(%u) OFF(%u)", hdr->seqNumber,
                        hdr->session, hdr->opcode, hdr->size, hdr->offset);
#endif
...
private:
    ...
    void handle_file_transfer_protocol(const mavlink::mavlink_message_t *msg, FTPRequest &req) {
        ...
        if (incoming_seqnr != expected_seqnr) {
            ROS_WARN_NAMED("ftp", "FTP: Lost sync! seqnr: %u != %u", incoming_seqnr,
                           expected_seqnr);
            go_idle(true, EILSEQ);
            return;
        }
    ...
    void handle_req_nack(FTPRequest &req) {
        ...
        ROS_ERROR_NAMED("ftp", "FTP: NAK: %u Opcode: %u State: %u Errno: %d (%s)", error_code,
                        hdr->req_opcode, enum_value(prev_op), r_errno, strerror(r_errno));
        go_idle(true);
    }
    ...
    void handle_ack_list(FTPRequest &req) {
        ...
        if (hdr->offset != list_offset) {
            ROS_ERROR_NAMED("ftp", "FTP: Wrong list offset, req %u, ret %u", list_offset,
                            hdr->offset);
            go_idle(true, EBADE);
            return;
        }
        ...
        while (off < hdr->size) {
            ...
            if (ptr[0] == FTPRequest::DIRENT_FILE || ptr[0] == FTPRequest::DIRENT_DIR) {
                add_dirent(ptr, slen);
    ...
    void handle_ack_open(FTPRequest &req) {
        ...
        ROS_INFO_NAMED("ftp", "FTP:Open %s: success, session %u, size %zu", open_path.c_str(),
                       hdr->session, open_size);
    ...
    void send_reset() {
        ...
        if (!session_file_map.empty()) {
            ROS_WARN_NAMED("ftp", "FTP: Reset closes %zu sessons", session_file_map.size());
    ...
    void send_list_command() {
        send_any_path_command(FTPRequest::kCmdListDirectory, "kCmdListDirectory: ", list_path,
                              list_offset);
    }
    ...
    void send_read_command() {
        ...
        ROS_DEBUG_STREAM_NAMED("ftp", "FTP:m: kCmdReadFile: " << active_session
                                                              << " off: " << read_offset);
    ...
    void send_write_command(const size_t bytes_to_copy) {
        ...
        ROS_DEBUG_STREAM_NAMED("ftp", "FTP:m: kCmdWriteFile: " << active_session
                                                               << " off: " << write_offset
                                                               << " sz: " << bytes_to_copy);
    ...
    bool read_file(std::string &path, size_t off, size_t len) {
        ...
        read_offset    = off;
        read_buffer.clear();
        if (read_buffer.capacity() < len || read_buffer.capacity() > len + MAX_RESERVE_DIFF) {
            // reserve memory
...
#define SERVICE_IDLE_CHECK()                                                                       \
    ...
    bool open_cb(mavros_msgs::FileOpen::Request &req, mavros_msgs::FileOpen::Response &res) {
        ...
        if (it != session_file_map.end()) {
            ROS_ERROR_NAMED("ftp", "FTP: File %s: already opened", req.file_path.c_str());
            return false;
        }
