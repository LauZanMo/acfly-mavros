...
public:
    ActuatorControlPlugin() : PluginBase(), nh("~") {}

    void initialize(UAS &uas_) override {
        PluginBase::initialize(uas_);

        target_actuator_control_pub =
            nh.advertise<mavros_msgs::ActuatorControl>("target_actuator_control", 10);
...
private:
    ...
    void handle_actuator_control_target(
        ...
        actuator_control_target_msg->header.stamp =
            m_uas->synchronise_stamp(actuator_control_target.time_usec);
    ...
    void actuator_control_cb(const mavros_msgs::ActuatorControl::ConstPtr &req) {
        ...
        std::copy(req->controls.begin(), req->controls.end(),
                  act.controls.begin()); // std::array = boost::array
