...
public:
    SetpointTrajectoryPlugin() : PluginBase(), sp_nh("~setpoint_trajectory") {}
    ...
    void initialize(UAS &uas_) override {
        ...
        mav_frame_srv =
            sp_nh.advertiseService("mav_frame", &SetpointTrajectoryPlugin::set_mav_frame_cb, this);

        sp_timer = sp_nh.createTimer(ros::Duration(0.01), &SetpointTrajectoryPlugin::reference_cb,
                                     this, true);
...
private:
    ...
    void reference_cb(const ros::TimerEvent &event) {
        ...
        set_position_target_local_ned(ros::Time::now().toNSec() / 1000000,
                                      utils::enum_value(mav_frame), type_mask, position, velocity,
                                      af, ftf::quaternion_get_yaw(attitude), yaw_rate);
