...
void MissionBase::handle_mission_item_int(const mavlink::mavlink_message_t *msg, WP_ITEM_INT &wpi) {
    ...
    // 仅在接收状态下接收任务
        ...
        ROS_DEBUG_NAMED(log_ns, "%s: rejecting item, wrong state %d", log_ns.c_str(),
                        enum_value(wp_state));
...
bool MissionBase::sequence_mismatch(const uint16_t &seq) {
    if (seq != wp_cur_id && seq != wp_cur_id + 1) {
        ROS_WARN_NAMED(log_ns, "%s: Seq mismatch, dropping request (%d != %zu)", log_ns.c_str(),
                       seq, wp_cur_id);
        return true;
    } else {
        return false;
...
void MissionBase::handle_mission_request(const mavlink::mavlink_message_t      *msg,
    ...
    if (mreq.mission_type != enum_value(wp_type)) {
        ...
        if (mreq.seq < wp_end_id) {
            ROS_DEBUG_NAMED(log_ns, "%s: FCU requested MISSION_ITEM waypoint %d", log_ns.c_str(),
                            mreq.seq);
            wp_cur_id = mreq.seq;
            if (use_mission_item_int) {
                ROS_DEBUG_NAMED(log_ns, "%s: Trying to send a MISSION_ITEM_INT instead",
                                log_ns.c_str());
                wp_state = WP::TXWPINT;
        ...
        ROS_DEBUG_NAMED(log_ns, "%s: rejecting request, wrong state %d", log_ns.c_str(),
                        enum_value(wp_state));
}
...
void MissionBase::handle_mission_request_int(const mavlink::mavlink_message_t          *msg,
    ...
    if (mreq.mission_type != enum_value(wp_type)) {
        ...
        restart_timeout_timer();
        if (mreq.seq < wp_end_id) {
            ROS_DEBUG_NAMED(log_ns, "%s: FCU reqested MISSION_ITEM_INT waypoint %d", log_ns.c_str(),
                            mreq.seq);
            wp_state  = WP::TXWPINT;
            wp_cur_id = mreq.seq;
            send_waypoint<WP_ITEM_INT>(wp_cur_id);
...
void MissionBase::handle_mission_ack(const mavlink::mavlink_message_t  *msg,
    ...
    if (mack.mission_type != enum_value(wp_type)) {
        ...
        if (ack_type != MRES::ACCEPTED) {
            is_timedout = true;
            lock.unlock();
            ROS_ERROR_STREAM_NAMED(log_ns,
                                   log_ns << ": clear failed: " << utils::to_string(ack_type));
        } else {
            waypoints.clear();
            lock.unlock();
            publish_waypoints();
...
void MissionBase::timeout_cb(const ros::TimerEvent &event) {
    ...
    if (wp_retries > 0) {
        ...
        if (wp_state == WP::TXWPINT && use_mission_item_int &&
            !mission_item_int_support_confirmed) {
            ROS_ERROR_NAMED(log_ns, "%s: mission_item_int timed out, falling back to mission_item.",
                            log_ns.c_str());
