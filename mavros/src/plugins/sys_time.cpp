...
public:
    TimeSyncStatus(const std::string &name, size_t win_size)
        : diagnostic_updater::DiagnosticTask(name), times_(win_size), seq_nums_(win_size),
          window_size_(win_size), min_freq_(0.01), max_freq_(10), tolerance_(0.1), last_rtt(0),
          rtt_sum(0), last_remote_ts(0), offset(0) {
        clear();
    }
...
private:
    ...
    inline bool sync_converged() { return sequence >= convergence_window; }
