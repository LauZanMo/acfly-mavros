...
public:
    WindEstimationPlugin() : PluginBase(), nh("~") {}
...
private:
    ...
    void handle_apm_wind(const mavlink::mavlink_message_t * msg,
        ...
        const double course =
            angles::from_degrees(wind.direction) + M_PI; // direction "from" -> direction "to"
    ...
    void handle_px4_wind(const mavlink::mavlink_message_t *msg,
        ...
        ftf::EigenMapCovariance6d cov_map(twist_cov->twist.covariance.data());
        cov_map.setZero();
        cov_map(0, 0) = wind.var_horiz; // NOTE: this is a summed covariance for both x and y
