...
public:
    ...
    int64_t to_integer() {
        ...
        // ]]]
        case XmlRpcValue::TypeBoolean:
            return static_cast<bool>(param_value);
        case XmlRpcValue::TypeInt:
            return static_cast<int32_t>(param_value);
    ...
    static bool check_exclude_param_id(std::string param_id) {
        return param_id == "SYSID_SW_MREV" || param_id == "SYS_NUM_RESETS" ||
               ...
               param_id == "GND_ABS_PRESS2" || param_id == "GND_ABS_PRESS3" ||
               param_id == "STAT_BOOTCNT" || param_id == "STAT_FLTTIME" ||
               param_id == "STAT_RESET" || param_id == "STAT_RUNTIME" || param_id == "GND_TEMP" ||
               param_id == "CMD_TOTAL" || param_id == "CMD_INDEX" || param_id == "LOG_LASTFILE" ||
               param_id == "FENCE_TOTAL" || param_id == "FORMAT_VERSION";
    }
};
...
public:
    ParamSetOpt(Parameter &_p, size_t _rem)
        : param(_p), retries_remaining(_rem), is_timedout(false) {}
...
private:
    ...
    void handle_param_value(const mavlink::mavlink_message_t  *msg,
        ...
        if (param_it != parameters.end()) {
            ...
            ROS_WARN_STREAM_COND_NAMED(
                ((p.param_index != pmsg.param_index && pmsg.param_index != UINT16_MAX) ||
        ...
        if (param_state == PR::RXLIST || param_state == PR::RXPARAM ||
            ...
            if (parameters_missing_idx.empty()) {
                ...
                ROS_DEBUG_NAMED("param", "PR: requesting next timed out parameter idx=%u",
                                first_miss_idx);
    ...
    void timeout_cb(const ros::TimerEvent &event) {
        ...
        if (param_state == PR::RXLIST && param_rx_retries > 0) {
            ...
            if (param_rx_retries > 0) {
                ...
                ROS_ERROR_NAMED("param", "PR: request param #%u completely missing.",
                                first_miss_idx);
            ...
            if (it->second->retries_remaining > 0) {
                ...
                ROS_WARN_NAMED("param", "PR: Resend param set for %s, retries left %zu",
                               it->second->param.param_id.c_str(), it->second->retries_remaining);
                restart_timeout_timer();
    ...
    bool wait_param_set_ack_for(std::shared_ptr<ParamSetOpt> opt) {
        ...
        return opt->ack.wait_for(lock, std::chrono::nanoseconds(PARAM_TIMEOUT_DT.toNSec()) *
                                           (RETRIES_COUNT + 2)) == std::cv_status::no_timeout &&
               !opt->is_timedout;
    }
    ...
    bool set_cb(mavros_msgs::ParamSet::Request &req, mavros_msgs::ParamSet::Response &res) {
        ...
        if (param_it != parameters.end()) {
            ...
            else if (param_it->second.param_value.getType() ==
