...
public:
    SetpointRawPlugin() : PluginBase(), sp_nh("~setpoint_raw") {}
    ...
    void initialize(UAS &uas_) override {
        ...
        if (!sp_nh.getParam("thrust_scaling", thrust_scaling)) {
            ROS_WARN_THROTTLE_NAMED(5, "setpoint_raw",
                                    "thrust_scaling parameter is unset. Attitude (and angular "
                                    "rate/thrust) setpoints will be ignored.");
            thrust_scaling = -1.0;
        }
    }
...
private:
    ...
    void handle_attitude_target(const mavlink::mavlink_message_t *     msg,
        ...
        auto orientation = ftf::transform_orientation_ned_enu(
            ftf::transform_orientation_baselink_aircraft(ftf::mavlink_to_quaternion(tgt.q)));

        auto body_rate = ftf::transform_frame_baselink_aircraft(
    ...
    void local_cb(const mavros_msgs::PositionTarget::ConstPtr &req) {
        ...
        if (req->coordinate_frame == mavros_msgs::PositionTarget::FRAME_BODY_NED ||
            ...
            yaw =
                ftf::quaternion_get_yaw(ftf::transform_orientation_absolute_frame_aircraft_baselink(
            ...
            yaw      = ftf::quaternion_get_yaw(ftf::transform_orientation_aircraft_baselink(
                ftf::transform_orientation_ned_enu(ftf::quaternion_from_rpy(0.0, 0.0, req->yaw))));
        }
        ...
        set_position_target_local_ned(req->header.stamp.toNSec() / 1000000, req->coordinate_frame,
                                      req->type_mask, position, velocity, af, yaw, yaw_rate);
    }
    ...
    void global_cb(const mavros_msgs::GlobalPositionTarget::ConstPtr &req) {
        ...
        set_position_target_global_int(req->header.stamp.toNSec() / 1000000, req->coordinate_frame,
                                       req->type_mask, req->latitude * 1e7, req->longitude * 1e7,
    ...
    void attitude_cb(const mavros_msgs::AttitudeTarget::ConstPtr &req) {
        ...
        if (ignore_thrust) {
            // I believe it's safer without sending zero thrust, but actually ignoring the
            // actuation.
            ROS_FATAL_THROTTLE_NAMED(5, "setpoint_raw",
