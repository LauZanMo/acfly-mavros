...
public:
    HomePositionPlugin()
        : hp_nh("~home_position"), REQUEST_POLL_TIME_DT(REQUEST_POLL_TIME_MS / 1000.0) {}
...
private:
    ...
    void handle_home_position(const mavlink::mavlink_message_t    *msg,
        ...
        auto hp_approach_enu = ftf::transform_frame_ned_enu(Eigen::Vector3d(
            home_position.approach_x, home_position.approach_y, home_position.approach_z));
        ...
        hp->geo.altitude =
            home_position.altitude / 1E3 + m_uas->geoid_to_ellipsoid_height(&hp->geo); // in meters
