...
static const cmode_map arduplane_cmode_map{
    {{0, "MANUAL"},  {1, "CIRCLE"},        {2, "STABILIZE"},   {3, "TRAINING"}, {4, "ACRO"},
     ...
     {11, "RTL"},    {12, "LOITER"},       {14, "LAND"},       // not in list
...
static const cmode_map arducopter_cmode_map{
    {{0, "STABILIZE"}, {1, "ACRO"},        {2, "ALT_HOLD"},     {3, "AUTO"},     {4, "GUIDED"},
     {5, "LOITER"},    {6, "RTL"},         {7, "CIRCLE"},       {8, "POSITION"}, // not in list
     {9, "LAND"},      {10, "OF_LOITER"},  {11, "DRIFT"}, // renamed, prev name: APPROACH
...
std::string UAS::str_mode_v10(uint8_t base_mode, uint32_t custom_mode) {
    ...
    if (MAV_AUTOPILOT::ARDUPILOTMEGA == ap) {
        if (is_apm_copter(type))
            ...
            return str_mode_cmap(
                apmrover2_cmode_map,
                custom_mode); // NOTE: #1051 for now (19.06.2018) boat is same as rover
        else if (type == MAV_TYPE::SUBMARINE)
            return str_mode_cmap(ardusub_cmode_map, custom_mode);
        else {
            ROS_WARN_THROTTLE_NAMED(30, "uas", "MODE: Unknown APM based FCU! Type: %d",
                                    enum_value(type));
            return str_custom_mode(custom_mode);
        }
