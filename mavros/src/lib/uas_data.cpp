...
UAS::UAS()
    ...
    try {
        ...
        ROS_FATAL_STREAM("UAS: GeographicLib exception: "
                         << e.what()
                         << " | Run install_geographiclib_dataset.sh script in order to install "
                            "Geoid Model dataset!");
        ros::shutdown();
    }
    ...
    add_static_transform("map", "map_ned",
                         Eigen::Affine3d(ftf::quaternion_from_rpy(M_PI, 0, M_PI_2)),
                         transform_vector);
    add_static_transform("odom", "odom_ned",
                         Eigen::Affine3d(ftf::quaternion_from_rpy(M_PI, 0, M_PI_2)),
                         transform_vector);
    add_static_transform("base_link", "base_link_frd",
