...
static const std::array<const std::string, 20> mav_autopilot_strings{{
    ...
    /*  5 */ "Generic autopilot only supporting simple waypoints", // Generic autopilot only
                                                                   // supporting simple waypoints
                                                                   /*  6 */
    "Generic autopilot supporting waypoints and other simple navigation commands", // Generic
                                                                                   // autopilot
                                                                                   // supporting
                                                                                   // waypoints and
                                                                                   // other simple
                                                                                   // navigation
                                                                                   // commands
    /*  7 */ "Generic autopilot supporting the full mission command set", // Generic autopilot
                                                                          // supporting the full
                                                                          // mission command set
...
static const std::array<const std::string, 36> mav_type_strings{{
    ...
    /* 19 */ "Two",  // Two-rotor VTOL using control surfaces in vertical operation in addition.
                     // Tailsitter.
    /* 20 */ "Quad", // Quad-rotor VTOL using a V-shaped quad config in vertical operation.
                     // Tailsitter.
    ...
    /* 33 */ "Servo",                            // Servo
    /* 34 */ "Open Drone ID. See https:",        // Open Drone ID. See
...
static const std::array<const std::string, 36> mav_type_names{{
    ...
    /* 19 */ "VTOL_DUOROTOR",      // Two-rotor VTOL using control surfaces in vertical operation in
                                   // addition. Tailsitter.
    /* 20 */ "VTOL_QUADROTOR",     // Quad-rotor VTOL using a V-shaped quad config in vertical
...
static const std::array<const std::string, 16> mav_mission_result_strings{{
    ...
    /*  1 */ "Generic error / not accepting mission commands at all right now.", // Generic error /
                                                                                 // not accepting
                                                                                 // mission commands
                                                                                 // at all right
                                                                                 // now.
    ...
    /*  5 */ "One of the parameters has an invalid value.", // One of the parameters has an invalid
                                                            // value.
    ...
    /* 14 */ "Not accepting any mission commands from this communication partner.", // Not accepting
                                                                                    // any mission
                                                                                    // commands from
                                                                                    // this
                                                                                    // communication
                                                                                    // partner.
                                                                                    /* 15 */
    "Current mission operation cancelled (e.g. mission upload, mission download).", // Current
                                                                                    // mission
                                                                                    // operation
                                                                                    // cancelled
                                                                                    // (e.g. mission
...
static const std::array<const std::string, 22> mav_frame_strings{{
    /*  0 */ "GLOBAL", // Global (WGS84) coordinate frame + MSL altitude. First value / x: latitude,
    ...
    /*  3 */ "GLOBAL_RELATIVE_ALT", // Global (WGS84) coordinate frame + altitude relative to the
    ...
    /*  5 */ "GLOBAL_INT", // Global (WGS84) coordinate frame (scaled) + MSL altitude. First value /
                           ...
                           // (MSL).
    /*  6 */ "GLOBAL_RELATIVE_ALT_INT", // Global (WGS84) coordinate frame (scaled) + altitude
                                        // relative to the home position. First value / x: latitude
                                        // in degrees*10e-7, second value / y: longitude in
                                        // degrees*10e-7, third value / z: positive altitude with 0
                                        // being at the altitude of the home location.
    /*  7 */ "LOCAL_OFFSET_NED", // Offset to the current local frame. Anything expressed in this
                                 // frame should be added to the current local frame position.
    /*  8 */ "BODY_NED", // Setpoint in body NED frame. This makes sense if all position control is
                         // externalized - e.g. useful to command 2 m/s^2 acceleration to the right.
    /*  9 */ "BODY_OFFSET_NED", // Offset in body NED frame. This makes sense if adding setpoints to
                                // the current flight path, to avoid an obstacle - e.g. useful to
                                // command 2 m/s^2 acceleration to the east.
    /* 10 */ "GLOBAL_TERRAIN_ALT", // Global (WGS84) coordinate frame with AGL altitude (at the
                                   // waypoint coordinate). First value / x: latitude in degrees,
                                   // second value / y: longitude in degrees, third value / z:
                                   // positive altitude in meters with 0 being at ground level in
                                   // terrain model.
    /* 11 */ "GLOBAL_TERRAIN_ALT_INT", // Global (WGS84) coordinate frame (scaled) with AGL altitude
                                       // (at the waypoint coordinate). First value / x: latitude in
                                       // degrees*10e-7, second value / y: longitude in
                                       // degrees*10e-7, third value / z: positive altitude in
    ...
    /* 13 */ "RESERVED_13", // MAV_FRAME_BODY_FLU - Body fixed frame of reference, Z-up (x: Forward,
                            // y: Left, z: Up).
    /* 14 */ "RESERVED_14", // MAV_FRAME_MOCAP_NED - Odometry local coordinate frame of data given
                            // by a motion capture system, Z-down (x: North, y: East, z: Down).
    /* 15 */ "RESERVED_15", // MAV_FRAME_MOCAP_ENU - Odometry local coordinate frame of data given
                            // by a motion capture system, Z-up (x: East, y: North, z: Up).
    /* 16 */ "RESERVED_16", // MAV_FRAME_VISION_NED - Odometry local coordinate frame of data given
                            // by a vision estimation system, Z-down (x: North, y: East, z: Down).
    /* 17 */ "RESERVED_17", // MAV_FRAME_VISION_ENU - Odometry local coordinate frame of data given
                            // by a vision estimation system, Z-up (x: East, y: North, z: Up).
    /* 18 */ "RESERVED_18", // MAV_FRAME_ESTIM_NED - Odometry local coordinate frame of data given
                            // by an estimator running onboard the vehicle, Z-down (x: North, y:
                            // East, z: Down).
...
// std::string> {suffix.lower()}_strings{{{{") for k, e in enum:
...
static const std::unordered_map<typename std::underlying_type<MAV_COMPONENT>::type,
                                const std::string>
    mav_comp_id_strings{{
        {0, "ALL"}, // Target id (target_component) used to broadcast messages to all components of
                    // the receiving system. Components should attempt to process messages with this
                    // component ID and forward to components on any other interfaces. Note: This is
                    // not a valid *source* component id for a message.
        {1, "AUTOPILOT1"}, // System flight controller component ("autopilot"). Only one autopilot
                           // is expected in a particular system.
        {25,
         "USER1"}, // Id for a component on privately managed MAVLink network. Can be used for any
                   // purpose but may not be published by components outside of the private network.
        {26,
         "USER2"}, // Id for a component on privately managed MAVLink network. Can be used for any
                   // purpose but may not be published by components outside of the private network.
        {27,
         "USER3"}, // Id for a component on privately managed MAVLink network. Can be used for any
                   // purpose but may not be published by components outside of the private network.
        {28,
         "USER4"}, // Id for a component on privately managed MAVLink network. Can be used for any
                   // purpose but may not be published by components outside of the private network.
        {29,
         "USER5"}, // Id for a component on privately managed MAVLink network. Can be used for any
                   // purpose but may not be published by components outside of the private network.
        {30,
         "USER6"}, // Id for a component on privately managed MAVLink network. Can be used for any
                   // purpose but may not be published by components outside of the private network.
        {31,
         "USER7"}, // Id for a component on privately managed MAVLink network. Can be used for any
                   // purpose but may not be published by components outside of the private network.
        {32,
         "USER8"}, // Id for a component on privately managed MAVLink network. Can be used for any
                   // purpose but may not be published by components outside of the private network.
        {33,
         "USER9"}, // Id for a component on privately managed MAVLink network. Can be used for any
                   // purpose but may not be published by components outside of the private network.
        {34, "USER10"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {35, "USER11"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {36, "USER12"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {37, "USER13"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {38, "USER14"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {39, "USER15"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {40, "USER16"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {41, "USER17"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {42, "USER18"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {43, "USER19"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {44, "USER20"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {45, "USER21"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {46, "USER22"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {47, "USER23"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {48, "USER24"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {49, "USER25"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {50, "USER26"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {51, "USER27"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {52, "USER28"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {53, "USER29"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {54, "USER30"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {55, "USER31"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {56, "USER32"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {57, "USER33"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {58, "USER34"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {59, "USER35"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {60, "USER36"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {61, "USER37"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {62, "USER38"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {63, "USER39"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {64, "USER40"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {65, "USER41"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {66, "USER42"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {67, "USER43"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {68, "TELEMETRY_RADIO"}, // Telemetry radio (e.g. SiK radio, or other component that emits
                                 // RADIO_STATUS messages).
        {69, "USER45"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {70, "USER46"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {71, "USER47"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {72, "USER48"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {73, "USER49"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {74, "USER50"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {75, "USER51"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {76, "USER52"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {77, "USER53"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {78, "USER54"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {79, "USER55"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {80, "USER56"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {81, "USER57"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {82, "USER58"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {83, "USER59"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {84, "USER60"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {85, "USER61"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {86, "USER62"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {87, "USER63"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {88, "USER64"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {89, "USER65"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {90, "USER66"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {91, "USER67"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {92, "USER68"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {93, "USER69"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {94, "USER70"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {95, "USER71"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {96, "USER72"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {97, "USER73"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {98, "USER74"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {99, "USER75"}, // Id for a component on privately managed MAVLink network. Can be used for
                        // any purpose but may not be published by components outside of the private
                        // network.
        {100, "CAMERA"},     // Camera #1.
        ...
        {197, "VISUAL_INERTIAL_ODOMETRY"}, // Component that provides position estimates using VIO
                                           // techniques.
        ...
        {242, "TUNNEL_NODE"}, // Component handling TUNNEL messages (e.g. vendor specific GUI of a
                              // component).
        {250,
         "SYSTEM_CONTROL"}, // Component for handling system messages (e.g. to ARM, takeoff, etc.).
    }};
...
LANDING_TARGET_TYPE landing_target_type_from_str(const std::string &landing_target_type) {
    ...
    ROS_ERROR_STREAM_NAMED("uas", "TYPE: Unknown LANDING_TARGET_TYPE: "
                                      << landing_target_type << ". Defaulting to LIGHT_BEACON");
