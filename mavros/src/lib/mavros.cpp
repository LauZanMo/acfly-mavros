...
MavRos::MavRos()
    ...
    mavlink_sub =
        mavlink_nh.subscribe("to", 100, &MavRos::mavlink_sub_cb, this,
                             ros::TransportHints().unreliable().maxDatagramSize(1024).reliable());

    // setup UAS and diag
    mav_uas.set_tgt(tgt_system_id, tgt_component_id);
    UAS_FCU(&mav_uas) = fcu_link;

    mav_uas.add_connection_change_handler(
        std::bind(&MavlinkDiag::set_connection_status, &fcu_link_diag, std::placeholders::_1));
    mav_uas.add_connection_change_handler(
        std::bind(&MavRos::log_connect_change, this, std::placeholders::_1));
    ...
    fcu_link->connect(
        [this](const mavlink_message_t *msg, const Framing framing) {
            ...
            if (gcs_link) {
                if (this->gcs_quiet_mode &&
                    msg->msgid != mavlink::minimal::msg::HEARTBEAT::MSG_ID &&
                    (ros::Time::now() - this->last_message_received_from_gcs >
                     this->conn_timeout)) {
                    return;
                }
    ...
    ROS_INFO("MAVROS started. MY ID %u.%u, TARGET ID %u.%u", system_id, component_id, tgt_system_id,
             tgt_component_id);
}
...
void MavRos::spin() {
    ...
    auto diag_timer = mavlink_nh.createTimer(ros::Duration(0.5), [&](const ros::TimerEvent &) {
    ...
    auto remote_endpoint_timer =
        mavlink_nh.createTimer(ros::Duration(1.0), [&](const ros::TimerEvent &) {
